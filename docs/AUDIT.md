...
| 6 | 🟠 HIGH | UI | Clics ratés en layout : `absX/absY` (hit-test ET rendu) pas recalculés après `UILayout` (calculés une fois au load, avant la 1re passe layout) | ✅ `<pending>` — `computeAbsolutePosition()` rappelé après `UILayout::layout()` dans `UIPanel::update` (fix chirurgical, pas de rewrite), locké par `UILayoutHitTestE2E` |
...
**Tier 3 (gros/stratégique)** : ✅ **harness E2E UI étendu** (`IT_016/UIClickAssert` : clic bouton + saisie textinput ; `IT_017/UIWidgetsE2E` : checkbox toggle + slider clic + **slider drag live** → **H2** ; `IT_018/UILayoutHitTestE2E` : clic sur widget positionné par layout vertical → **#6**). #5-texte + H2 + #6 corrigés. Restes : ScrollPanel (rework), mapping scancode des touches spéciales (#5 suite) · #4 `layer`→clé de tri submit · #10 `queryModule` via IIO + statut doc ThreadedModuleSystem · plafond reload (host out-of-process) · E2E pixel screenshot-diff · #7 unification matchers + #9 shared_lock (TSAN).
...
clic + **drag live** (`ui:value_changed{value}` à chaque frame du drag), et **hit-test sur widget
positionné par layout vertical** (`ui:action` du bon enfant). Restent non prouvés : scroll,
