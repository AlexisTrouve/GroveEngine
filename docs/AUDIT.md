...
| 4 | 🟠 HIGH | 2D | Layer/depth pas honoré au submit (depth=0 partout) : ordre = ordre CPU intra-pass (retained en ordre de hash !), pas le champ `layer` | ✅ `a95e479` — `finalize()` `stable_sort` par `layer` (sprites + textes) → paquet déterministe = z-order ; locké par `SceneCollectorTest`. Nuance vérifiée : `SpritePass` re-triait déjà les sprites par layer au submit, donc le gain net porte sur les **textes** (`TextPass` ne trie pas) + l'ordre déterministe du paquet. Reste : ordre **inter-passes** (sprite vs texte vs tilemap, fixé par le RenderGraph, pas par `layer`) + couche bgfx réelle / pixel non assertés |
...
| SP | 🟠 HIGH | UI | ScrollPanel : `render()` posait child->absX scrollé puis le RESTAURAIT non-scrollé → rendu scrollé mais hit-test non-scrollé → clic sur enfant scrollé raté | 🟡 **hit-test corrigé** `b3a0c72` — scroll persistant dans `absX` (posé dans `update()`), locké par `UIScrollPanelE2E` ; **scissor/clipping ✅ livré** depuis (`IT_024`/`IT_025` côté UI + `SpriteClipGpu`/`TextClipGpu` GPU). Résidu étroit : un enfant panel non-absolu se dé-scrolle (rework scrollpanel futur) |
| 7 | 🟠 HIGH | IIO | 3 matchers de patterns incohérents (regex IntraIO ≠ lambda manager ≠ TopicTree) → mis-routing / swallow | 🟢 **fonctionnellement résolu** `c15d812` + lock E2E — les 2 matchers qui décident la livraison (TopicTree routage + lambda `patternMatches` du manager) sont cohérents (`*`=1 segment, `.*`=terminal), prouvé end-to-end par `IIOBackpressure` ; le regex per-instance reste cosmétiquement plus laxiste sur `*` simple mais **masqué** par TopicTree (filtre post-routage : peut enlever, jamais ajouter une livraison) — unification du 3e matcher = cosmétique, non requise |
...
| 10 | 🟠 HIGH | Threaded | BUG D réel : `queryModule` appelle `process()` depuis le thread appelant pendant le worker → data race, juste loggée | ✅ `e950459` — `processMutex` **par-worker** sérialise les 2 appels `process()` (exclusion mutuelle par construction, parallélisme inter-modules préservé), locké par `ThreadedQueryRace`. Le race BUG D est éliminé ; un audit TSAN complet du système threadé reste recommandé |
