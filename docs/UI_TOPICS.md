...
| `ui:list:set_tree` | `{id, nodes:[{id,label,collapsed?,icon?,children?:[…]}]}` | Repopulate a `list` as an N-LEVEL TREE (slice 5d) — a node with `children` is a collapsible header, a leaf is a selectable item; each level indents. Headers fire `ui:list:group:toggled`, leaves `ui:list:selected`. Same json-backed constraint (the recursive `nodes`/`children` arrays must live in the node's JSON) |
