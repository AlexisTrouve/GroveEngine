...
| 2026-06-22 | 3b-2 Window interaction | ✅ SHIPPED. `UIModule::handleWindowInteraction()` (centralized on the topmost window under the cursor): **raise-on-click** (`bringToFront`), **title-bar drag**, **close** (hide + purge + `ui:window:closed`). Locked by `IT_027` (raise flips the z-order) + `IT_028` (drag moves it, close hides it). **Slice 3 (z-order + in-app Window) COMPLETE.** Deferred: window **resize**. Next: drawers / tabs / list-grid / modal. |
