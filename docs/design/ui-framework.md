...
| 2026-06-22 | 3a z-order primitive | ✅ SHIPPED. `UIWidget::bringToFront()` — z-order = sibling order (last = top + hit first), so raising = move to the back of `parent->children`. Locked by `UIZOrderUnit`. Next 3b: in-app **Window** widget (titlebar drag / close / raise-on-click / clipped) built on this + slice-2 clipping. NB: in-app windows, NOT OS windows (that's the deferred multi-screen item). |
