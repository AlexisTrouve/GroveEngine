...
| **5e** | **List/Grid view** (repeater + select) — ✅ MVP SHIPPED; virtual scroll deferred | 1 + 2 | E2E + perf | partial ship sidebar |
...
| 2026-06-22 | 5e List/Sidebar MVP | ✅ SHIPPED (MVP; virtualization deferred). `UIList` — the **ship sidebar**: data-driven (`items[{id,label,subtitle?,icon?}]`) scrollable/clipped/selectable list. Self-managed retained row-id pool (NOT child widgets → sidesteps the UIScrollPanel re-layout-descroll limitation), wheel scroll (routed like scrollpanel), opaque hit-test absorb, scroll-aware `rowAt()`, single-select → `ui:list:selected {id,index,itemId}`. Runtime `ui:list:set_items` (repopulate, purges the old pool) + `ui:list:select`. Locked by `IT_033` (select / scroll+clip click-flip / runtime repopulate) + `UIListUnit` (rowAt / scroll clamp / parseItems / selection). **Surfaced a real engine fact: IIO transports only a node's JSON (`m_data`) — array payloads must be json-backed, not `setChild`-assembled** (documented in UI_TOPICS §note + handoff §7). Deliberate follow-ons (the "virtualized" half of 5e): virtualization (O(visible) not O(N)/frame), visual scrollbar + drag-scroll, row templates, multi-select. |
