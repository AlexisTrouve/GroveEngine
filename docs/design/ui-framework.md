...
| **6c** ✅ | **Video panel** — ✅ SHIPPED. `VideoModule` + A/V sync (`VideoSync`) + raw-pixel `render:texture:upload` + **`FfmpegCliBackend` (real MP4/H.264/AAC via the ffmpeg CLI subprocess)** | 6a+6b | `VideoSyncUnit`+`IT_058`+`RuntimeTextureGpu`+`FfmpegBackendReal` | video scene (MP4 + audio) |
...
| Date | Slice | State |
|---|---|---|
| 2026-07-06 | 7 VN conditions | ✅ SHIPPED — a VN follow-on. `DialogueRuntime` gains a variable store + **declarative conditions** (NOT an expression language, matching the binding doctrine): a node `set:{var:value}` assigns on entry; a `choice.when:[{var,op?,value?}]` gates whether it's OFFERED (a LIST of predicates = AND); a node `branch:[{when?,goto}]` picks the first matching linear edge on `advance`; `vars` seeds the store. Predicates are a single comparison on ONE variable (op ∈ truthy/falsy/eq/ne/gt/ge/lt/le) — no OR, no nesting, no parsing (compound = restructure the graph / derive a flag game-side). `availableChoices()` filters gated-out choices; `choose(i)` indexes the offered list; `DialogueModule` presents only offered choices + gains `scene:set {var,value}` (re-presents so an unlocked choice appears live) + preserves vars across hot-reload. Locked by `DialogueRuntimeUnit [conditions]` (gate/set/branch/choose-indexes-available; **prove-it-bites**: never-gate → RED) + `IT_057c` (through the module). |
| 2026-07-05 | 6c-1 MP4 decode | ✅ SHIPPED — **video is COMPLETE**. `FfmpegCliBackend` (`IVideoBackend`) decodes a real MP4/H.264/AAC by driving the **ffmpeg CLI as a subprocess** (Alexi's pick — no libav linking; ffmpeg.exe already on the box, bundle it to ship): ffprobe → geometry/fps/frameCount, a long-running `ffmpeg … -f rawvideo -pix_fmt rgba -` pipe → RGBA frames (forward-only, drops skipped frames), and the audio track extracted to a temp OGG → the sound system plays it as the master clock. `popen` (portable `_popen`/`popen`). Added to `VideoModule_static`. Locked by `FfmpegBackendReal` (generates a testsrc MP4, decodes it, asserts geometry/fps/frame-count/real-pixels; gated on ffmpeg on PATH, WARN-skips otherwise). **Full chain now proven maillon-by-maillon**: FfmpegCliBackend (real MP4) → VideoModule (audio-master sync, `IT_058`) → `render:texture:upload` (GPU pixels, `RuntimeTextureGpu`). **By-eye+ear demo SHIPPED** (`tests/visual/test_video_demo.cpp`): wires BgfxRenderer + SoundManager + VideoModule in a window, plays a real MP4 (generated testsrc+sine, or `argv[1]`) with synced audio to the end — verified running (clean `video:ended` + teardown). **Smooth playback**: the module DEAD-RECKONS the audio clock (advance by dt each frame, SNAP to `sound:music:position` when it arrives ~15 Hz) so the picture is fluid 60 fps locked to the audio, not stepping at the 15 Hz position rate. Follow-on: seek. |
