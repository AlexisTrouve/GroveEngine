/**
 * Headless CAPTURE of the UI rendered through the ENGINE — to a PNG.
 *
 * The visual half of the "UI runs on the engine" proof (the headless logic lock is
 * IT_053). This drives the SAME real pipeline as capture_ui_demo.cpp (UIModule +
 * BgfxRendererModule on a hidden bgfx window, the Fleet Command screen), but BOTH
 * modules are HOSTED BY THE ENGINE: registered via IEngine::registerStaticModule()
 * and driven by engine.step() — no hand-wired process(), no manual IIO routing. For
 * the capture frame the renderer's views are redirected into an offscreen
 * framebuffer, rendered, read back, and written as a PNG. Lets us SEE exactly what
 * the engine-hosted UI draws, headless.
 *
 * Usage: capture_ui_on_engine [out.png]   (run from the project root for the asset path)
 */
...
int main(int argc, char** argv) {
    const std::string outPath = argc > 1 ? argv[1] : "demo_capture_on_engine.png";
    const int W = 1280, H = 720;

    SDL_SetMainReady();
    if (SDL_Init(SDL_INIT_VIDEO) != 0) { std::fprintf(stderr, "no SDL: %s\n", SDL_GetError()); return 1; }
    SDL_Window* win = SDL_CreateWindow("capture_on_engine", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, W, H, SDL_WINDOW_HIDDEN);
    if (!win) { std::fprintf(stderr, "no window\n"); SDL_Quit(); return 1; }
    SDL_SysWMinfo wmi; SDL_VERSION(&wmi.version); SDL_GetWindowWMInfo(win, &wmi);

    // The "game side" instance: injects data + the camera. The renderer and UI are
    // NOT created here — the engine creates their routed IIO instances internally.
    ...
    // HOST the UI on the engine. Order matters: UI is registered BEFORE the renderer
    // so each frame UIModule.process() publishes its render:* commands before the
    // renderer processes (same-frame draw). registerStaticModule wires each module's
    // setConfiguration(config, io, scheduler) — no hand-wiring.
    {
        auto uiCfg = std::make_unique<JsonDataNode>("config");
        uiCfg->setString("layoutFile", "assets/ui/demo_fleet_command.json");
    ...
    // Keep a raw pointer to the renderer (for the offscreen device) BEFORE moving it in.
    ...
    // A frame = publish the camera, then engine.step() — the ENGINE runs ui then renderer.
    auto frame = [&]{
    ...
    // Populate + select a ship so the detail window shows real content (same as capture_ui_demo).
    ...
    std::fclose(fp);
    std::fprintf(stdout, "wrote %s (%dx%d) — UI rendered THROUGH the engine\n", outPath.c_str(), W, H);

    engine.shutdown();
    mgr.removeInstance("cap_game");
