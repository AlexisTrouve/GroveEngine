...
- **Adaptive music (slices 1-2)**: `audio:*` (declarative mood) — `audio:layer {id,path,gainCalm?,gainPeak?,loop?}` (stem; gain crossfades calm→peak by tension), `audio:intent {tension, quantize?}` (0..1; `quantize` "now"/"bar"/"beat"), `audio:tempo {bpm,beatsPerBar?}`, `audio:mix {id,gain}`, `audio:layer:stop {id,fadeMs?}`. Pure `AdaptiveMixer` ramps stem gains (music bus) via `ISoundBackend::setSoundVolume`; pure `BeatClock` quantizes transitions to the measure. Locked by `SoundManagerUnit` `[adaptive]`. Next: cues/stingers, leitmotifs.
