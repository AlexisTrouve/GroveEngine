...
- **Camera helper**: `Scene/Camera.h` (`grove::camera`, header-only) — screen↔world, centerOn/focusOn, `zoomAt` (seamless zoom toward cursor), `damp`, **`isVisible`/`visibleWorldBounds` (cull off-screen: game skips submit + rotation/anim compute; renderer can cull draws)**. Camera `(x,y)` = world coord at viewport **top-left**; `screen = zoom·(world−cam)`. Locked by `CameraUnit` + `SceneCollectorTest` (matrices match the helper)
...
- **Adaptive music (slices 1-2)**: `audio:*` (declarative mood) — `audio:layer {id,path,gainCalm?,gainPeak?,loop?}` (stem; gain crossfades calm→peak by tension), `audio:intent {tension, quantize?}` (0..1; `quantize` "now"/"bar"/"beat"), `audio:tempo {bpm,beatsPerBar?}`, `audio:mix {id,gain}`, `audio:cue {path,volume?,quantize?}` (one-shot sting, quantizable), `audio:layer:stop {id,fadeMs?}`. Pure `AdaptiveMixer` ramps stem gains (music bus) via `ISoundBackend::setSoundVolume`; pure `BeatClock` quantizes transitions to the measure. Locked by `SoundManagerUnit` `[adaptive]`. Next: leitmotif-by-entity-state.
