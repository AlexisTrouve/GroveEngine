...
- **2D lighting** (`render:ambient` + `render:light`): the world renders to an offscreen RGBA16F target, radial lights accumulate ADDITIVELY into a second one, and a full-screen pass composites `final = scene × (ambient + light) + light × scatter`. Entry doc: **`docs/design/lighting-2d.md`** (it links the rest).
  ...
  - **⚠️ Budget: NOT re-measured since the march landed.** Each lamp is one quad costing up to 64 texture taps per covered pixel, so *small* lamps are far cheaper than large ones. The "tens of lamps" figure that used to sit here predates the occlusion march — treat it as unverified until a bench exists.
  ...
  - **Docs**: `lighting-2d.md` (entry) · `lighting-transmittance-core.md` (the shared mechanism) · `lighting-walls.md` · `lighting-filters.md` · `lighting-attenuators.md`. Visuals + GIFs: `blog/` (see `IMAGES.md`).
