...
- **ZoneNavigator** (`Scene/ZoneNavigator.h`, header-only, `grove::camera`): nested-zones navigation — the game syncs a tree of zones (id/parent/bounds), the engine drives the camera: zoom-to-cursor *enters* a zone, pan locked+scaled+camera-frame, soft magnet, **per-layer zoom bounds**, seamless deletion back-out, **lock onto a moving zone**, camera roll. `configure` knobs: margin/magnetRate/panMargin/maxDetail. Per frame: feed input → `update(dt)` → publish `render:camera`. Locked by `ZoneNavUnit`. Docs: DEVELOPER_GUIDE + `docs/design/zone-navigation.md`. Also `ZoomLadder` (`Scene/ZoomLadder.h`) for content-less plateau zoom
