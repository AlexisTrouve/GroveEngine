...
- **2D lighting** (`render:ambient` + `render:light`): the world renders to an offscreen RGBA16F target, radial lights accumulate ADDITIVELY into a second one, and a full-screen pass composites `final = scene × (ambient + light)`. **⚠️ Nothing is lit until `render:ambient` is published** — absent/0 means NO offscreen target, NO composite, output byte-identical to a build without lighting (the zero-cost bypass every current consumer relies on). A **white** ambient leaves the scene unchanged and lets lights only brighten it — you don't need a dark game. Lights are **ephemeral** (re-publish per frame), `cx,cy` = CENTRE, `radius` in world units, `intensity` may exceed 1 (RGBA16F keeps the overbright for a future bloom). Falloff `(1−d/r)²`, exactly 0 at the rim; the same curve is plain C++ in **`grove::light`** (`include/grove/light/Light.h`) so gameplay can ask "is this point lit?" without a readback. Designed for **tens** of lights (one draw each); the wall would be the IIO path, not the GPU. **Cone lights** via optional `dirDeg`/`spreadDeg` (degrees, `grove::fx::Emitter` convention so a thruster's emitter and its light share the same numbers; **360 = omni = the default**, so a light written before cones is unchanged); soft rim, evaluated on a dot product not `atan2`. **Walls cast shadows** (`render:occluder {x,y,w,h}` ephemeral + `:add`/`:update`/`:remove` retained — `x,y` = CORNER; retained is the right form for level geometry, the OPPOSITE advice to lights and for the opposite reason: a wall doesn't move): occluders draw BLACK into a screen-space occlusion map that the light shader MARCHES through, accumulating transmittance. A wall is no special case — it writes 0, and a zero annihilates the running product, so everything beyond it darkens arithmetically. Same mechanism will carry coloured filters + fog (`docs/design/lighting-transmittance-core.md` + the W/F/A plans). Pure oracle: `grove::light::accumulate` (`include/grove/light/Transmittance.h`, `TransmittanceUnit`). ⚠️ Occluders block LIGHT, not SIGHT. NO soft/penumbra shadows. Locked by `LightMathUnit` + `SceneCollectorTest [light]` + `PipelineHeadless [light]` + `LightingGpu` (pixel readback: ambient 1.0→255 / 0.5→128; a lamp under a panned+zoomed camera → centre 255 / corners 48). Docs: `docs/design/lighting-2d.md`
