...
- **Topics consumed**: `entity:spawn {id, transform?, sprite?, behaviors?:[{type,...}]}`, `entity:set {id, transform?, sprite?}` (partial-merge — omitted fields keep their value; robust JSON accessors fail-soft, never throw on a bad payload), `entity:destroy {id}`. **Published**: `render:sprite:add`/`:update`/`:remove` (`renderId` = the entity id; `cx,cy` = CENTER, the anchor convention). Each `process(dt)`: drain → `tick(dt)` → `diffRender()` → emit the minimal retained-render traffic.
- Static-link host (Drifterra): link **`EntityModule_static`**, drive `module.world()` directly (spawn/setSprite/addBehavior) then `process(dt)` each frame to tick + emit — or push `entity:*` topics. Locked by `EntityWorldUnit` (pure oracle) + `IT_059` (E2E through the module, prove-it-bites). Follow-ons: **E3 archetypes/prefabs** (named entity templates + spawn-with-overrides), more behaviors (follow/oscillate/path), text/particle components, hot-reload full-world serialization (getState is minimal for now).
