...
| **EntityModule** | ✅ Slice E2 | Data-driven scene/entity layer — compose entities+components+**behaviors** in data, the engine ticks them → `render:sprite:*`. Behaviors live **engine-side** (reused across projects). `entity:*` topics + C++ `world()` API | `-DGROVE_BUILD_ENTITY_MODULE=ON` (SDL-free) |
...
### EntityModule (`modules/EntityModule/`, `-DGROVE_BUILD_ENTITY_MODULE=ON`) — data-driven scene/entity layer (slice E2)
- The **multi-project authoring surface**: an entity = a stable id + typed **components** (`Transform{cx,cy,rotation,scaleX,scaleY}`, `Sprite{asset|textureId,color,layer}`) + a list of **behaviors** from a **FIXED engine library**. Pure header-only core `grove::entity::EntityWorld` (`include/grove/entity/EntityWorld.h`, like `grove::anim`/`DialogueRuntime`) does the tick + the retained-render diff; `EntityModule` (IModule) wraps it onto IIO.
- **Behaviors are engine-side → every project inherits them for free** (that's the multi-project point), WITHOUT a scripting language (a behavior is an enum of primitives with numeric params, like the VN conditions). Starting library: **`move{vx,vy}` · `spin{degPerSec}` · `lifetime{seconds}`**. Add a reusable behavior = add a `Type` + tick case in `EntityWorld` (all projects gain it). Bespoke game logic stays **consumer-side** (mutate components).
- **Archetypes/prefabs (E3)** — the multi-project entity-template library: `entity:prefab {name, transform?, sprite?, behaviors?}` registers a reusable template once; `entity:spawn {id, archetype:"bullet", ...}` instantiates it, the spawn's own transform/sprite MERGE on top (per-instance overrides) + its behaviors ADD to the prefab's. Each instance is a deep copy (fresh behavior state). Pure `registerPrefab`/`spawnFromPrefab` in `EntityWorld`; unknown archetype = fail-soft plain spawn. Locked by `EntityWorldUnit [prefab]` + `IT_059c`.
- **Topics consumed**: `entity:prefab {name,...}`, `entity:spawn {id, archetype?, transform?, sprite?, behaviors?:[{type,...}]}`, `entity:set {id, transform?, sprite?}` (partial-merge — omitted fields keep their value; robust JSON accessors fail-soft, never throw on a bad payload), `entity:destroy {id}`. **Published**: `render:sprite:add`/`:update`/`:remove` (`renderId` = the entity id; `cx,cy` = CENTER, the anchor convention). Each `process(dt)`: drain → `tick(dt)` → `diffRender()` → emit the minimal retained-render traffic.
- Static-link host (Drifterra): link **`EntityModule_static`**, drive `module.world()` directly (spawn/spawnFromPrefab/setSprite/addBehavior) then `process(dt)` each frame to tick + emit — or push `entity:*` topics. Locked by `EntityWorldUnit` (pure oracle) + `IT_059` (E2E through the module, prove-it-bites). Follow-ons: more behaviors (follow/oscillate/path), text/particle components, hot-reload full-world serialization (getState is minimal for now).
